...
def computeKeypointsAndDescriptors(image,num_octaves=4, sigma=1.6, S=3, image_border_width=5):
    ...
    gaussian_Scales = generat_Scales(sigma,S)
    gaussian_images = generate_Octaves(image,sigma,num_octaves, gaussian_Scales)
    dog_images = generateDoGImages(gaussian_images)
    keypoints = findScaleSpaceExtrema(gaussian_images, dog_images, S, sigma, image_border_width)
...
def generat_Scales(sigma,S):
    ...
    k = 2 ** (0.5)
    gaussian_Scales = zeros(scales)  # scale of gaussian blur necessary to go from one blur scale to the next within an octave
        ...
        for image_index, (first_image, second_image, third_image) in enumerate(zip(dog_images_in_octave, dog_images_in_octave[1:], dog_images_in_octave[2:])):
            ...
            for i in range(image_border_width, first_image.shape[0] - image_border_width):
                for j in range(image_border_width, first_image.shape[1] - image_border_width):
                    if key_points(first_image[i-1:i+2, j-1:j+2], second_image[i-1:i+2, j-1:j+2], third_image[i-1:i+2, j-1:j+2], threshold):
                        localization_result = localizeExtremumViaQuadraticFit(i, j, image_index + 1, octave_index, S, dog_images_in_octave, sigma, contrast_threshold, image_border_width)
...
def localizeExtremumViaQuadraticFit(i, j, image_index, octave_index, S, dog_images_in_octave, sigma, contrast_threshold, image_border_width, eigenvalue_ratio=10, num_attempts_until_convergence=5):
    ...
    for attempt_index in range(num_attempts_until_convergence):
        ...
        if i < image_border_width or i >= image_shape[0] - image_border_width or j < image_border_width or j >= image_shape[1] - image_border_width or image_index < 1 or image_index > S:
    ...
    if abs(functionValueAtUpdatedExtremum) * S >= contrast_threshold:
        ...
        if xy_hessian_det > 0 and eigenvalue_ratio * (xy_hessian_trace ** 2) < ((eigenvalue_ratio + 1) ** 2) * xy_hessian_det:
            ...
            keypoint.size = sigma * (2 ** ((image_index + extremum_update[2]) / float32(S))) * (2 ** (octave_index + 1))  # octave_index + 1 because the input image was doubled
